...
function buildMagnaDoppler(m) {
  ...
  // Safenectomia total: safena não existe mais, sem linha de refluxo
  if (m.safenectomiaMagna === "total") return [];
  const segLabel = (k) => SEG_LABEL[k] || k;
  // "residual" quando safenectomia parcial
...
function buildParvaDoppler(m) {
  ...
  // Safenectomia total: safena não existe mais, sem linha de refluxo
  if (m.safenectomiaParva === "total") return [];
  const segLabel = (k) => SEG_LABEL[k] || k;
...
function buildDopplerCotoParva(m) {
  if (m.safenectomiaParva === "none") return [];
  return [
    m.cotoParvaRefluxo
      ? "Coto residual da veia safena parva com refluxo."
      : "Coto residual da veia safena parva sem refluxo.",
  ];
}
...
function buildConclusaoMagna(m) {
  ...
  if (m.safenectomiaMagna === "total") {
    // 1º safenectomia, 2º coto, sem linha de refluxo (veia não existe)
  ...
  // 3º refluxo da veia (residual ou normal)
...
async function exportDocx(state, patientName, examDate) {
  ...
  function blockToParagraphs(lines, member) {
    ...
    lines.forEach((line) => {
      if (line === "__TABLE_MAGNA__") {
        ...
        // sem parágrafo extra após a tabela magna
  ...
  function buildSectionChildren(block) {
    ...
    // 1 parágrafo simples antes do DOPPLER (não emptyLine)
    sc.push(new Paragraph({ children: [], spacing: { after: SP } }));
    sc.push(...blockToParagraphs(block.doppler, block.member));

    // 1 parágrafo simples antes da CONCLUSÃO (não emptyLine)
    sc.push(new Paragraph({ children: [], spacing: { after: SP } }));
    sc.push(new Paragraph({
      spacing: { after: SP },
...
export default function AppMapeamento() {
  ...
  const buildReportLines = useCallback((onlySide) => {
    ...
    blocks.forEach((b) => {
      // Cabeçalho do membro
      lines.push(`MEMBRO INFERIOR ${SIDE_LABEL[b.side]}`, "");

      // Anatômico
      b.anatomico.slice(2).forEach((l) => {
  ...
  const buildReportHTML = useCallback((onlySide) => {
    ...
    blocks.forEach((b) => {
      // Cabeçalho do membro
      html += `<p style="${boldStyle}">MEMBRO INFERIOR ${SIDE_LABEL[b.side]}</p>`;
      html += `<p style="${pStyle}">&nbsp;</p>`;
